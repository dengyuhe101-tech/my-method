...
static PyObject* py_channel_gate(PyObject* self, PyObject *args, PyObject *kwargs)
{
	setlocale(LC_ALL, "C");
	int prev_layer = -1, network_id = 0, current_layer_id = -1;
	const char *activation = "LIN";
	static char *kwlist[] = {"activation", "prev_layer", "network", NULL};
	layer* prev;

	if(!PyArg_ParseTupleAndKeywords(args, kwargs, "|sii", kwlist, &activation, &prev_layer, &network_id))
		return PyLong_FromLong(-1);
	...
	current_layer_id = chgate_create(networks[network_id], prev, activation, NULL, 0);
...
static PyObject* py_set_yolo_params(PyObject* self, PyObject *args, PyObject *kwargs)
{
	...
	int nb_angle = 0, fit_angle = 1, angle_loss_mode = 0, target_box_mode = 0, obb_loss_mode = 0, multi_pos_topk = 1, prob_quality_mode = 0, obj_quality_mode = 0;
	double rand_prob_best_box_assoc = 0.0f, rand_prob = 0.0f, min_prior_forced_scaling = 0.0f;
	double angle_scale = 1.0, angle_unit_norm_scale = 0.0, angle_slope = 1.0, angle_fmax = 1.2, angle_fmin = -1.2, min_angle_IoU_lim = -2.0;
	double obb_loss_scale = 0.0, multi_pos_iou_ratio = 0.60, multi_pos_min_iou = 0.05, multi_pos_obj_weight = 0.35;
	double prob_quality_scale = 1.0, prob_quality_floor = 0.02;
	double obj_quality_scale = 1.0, obj_quality_floor = 0.02;
		double obj_quality_center_weight = 0.60, obj_quality_geom_weight = 0.25, obj_quality_phys_weight = 0.15;
		double obj_quality_da_center_weight = 0.65, obj_quality_da_geom_weight = 0.35, obj_quality_da_phys_weight = 0.0;
		double obj_quality_da_flux_max = 0.20, obj_quality_da_aspect_min = 1.5, obj_quality_da_aspect_max = 3.0;
		double obj_quality_da_bmaj_log_max = 0.0, obj_quality_da_bmaj_log_min = 0.0, obj_quality_da_bmin_log_max = 0.0, obj_quality_da_bmin_log_min = 0.0;
		double scorer_aux_flux_log_max = 0.0, scorer_aux_flux_log_min = 0.0;
		int scorer_aux_mode = 0;
		double scorer_aux_center_scale = 0.0, scorer_aux_flux_scale = 0.0, scorer_aux_size_scale = 0.0;
		double scorer_aux_da_weight = 1.0, scorer_aux_pixel_arcsec = 0.6042492, scorer_aux_beam_arcsec = 0.625;
		int flux_refine_mode = 0, flux_refine_delta_param_index = 3, flux_refine_gate_param_index = 4, flux_refine_detach_base = 1;
		double flux_refine_loss_scale = 0.0, flux_refine_gate_loss_scale = 0.0, flux_refine_final_loss_scale = 0.0;
		double flux_refine_delta_norm_scale = 0.25, flux_refine_gate_margin_norm = 0.01;
	...
	static char *kwlist[] = {"nb_box", "nb_class", "nb_param", "max_nb_obj_per_image", "prior_size", "prior_noobj_prob", "error_scales", "slopes_and_maxes", "param_ind_scales", "IoU_limits",
		...
		"min_prior_forced_scaling", "class_softmax", "diff_flag", "network", "error_type", "no_override", "raw_output",
				"nb_angle", "angle_scale", "angle_unit_norm_scale", "angle_slope", "angle_fmax", "angle_fmin", "min_angle_IoU_lim", "fit_angle", "angle_loss_mode",
				"target_box_mode", "obb_loss_mode", "obb_loss_scale",
				"multi_pos_topk", "multi_pos_iou_ratio", "multi_pos_min_iou", "multi_pos_obj_weight",
				"prob_quality_mode", "prob_quality_scale", "prob_quality_floor",
				"obj_quality_mode", "obj_quality_scale", "obj_quality_floor",
				"obj_quality_center_weight", "obj_quality_geom_weight", "obj_quality_phys_weight",
				"obj_quality_da_center_weight", "obj_quality_da_geom_weight", "obj_quality_da_phys_weight",
					"obj_quality_da_flux_max", "obj_quality_da_aspect_min", "obj_quality_da_aspect_max",
					"obj_quality_da_bmaj_log_max", "obj_quality_da_bmaj_log_min",
					"obj_quality_da_bmin_log_max", "obj_quality_da_bmin_log_min",
					"scorer_aux_flux_log_max", "scorer_aux_flux_log_min",
					"scorer_aux_mode", "scorer_aux_center_scale", "scorer_aux_flux_scale", "scorer_aux_size_scale",
					"scorer_aux_da_weight", "scorer_aux_pixel_arcsec", "scorer_aux_beam_arcsec",
					"flux_refine_mode", "flux_refine_delta_param_index", "flux_refine_gate_param_index",
					"flux_refine_detach_base", "flux_refine_loss_scale", "flux_refine_gate_loss_scale",
					"flux_refine_final_loss_scale", "flux_refine_delta_norm_scale", "flux_refine_gate_margin_norm", NULL};

					if(!PyArg_ParseTupleAndKeywords(args, kwargs, "|iiiiOOOOOOOssiiidddiiisiiiddddddiiiididddiddidddddddddddddddddiddddddiiiiddddd", kwlist,
					...
					&nb_angle, &angle_scale, &angle_unit_norm_scale, &angle_slope, &angle_fmax, &angle_fmin, &min_angle_IoU_lim, &fit_angle,
					&angle_loss_mode, &target_box_mode, &obb_loss_mode, &obb_loss_scale,
					&multi_pos_topk, &multi_pos_iou_ratio, &multi_pos_min_iou, &multi_pos_obj_weight,
					&prob_quality_mode, &prob_quality_scale, &prob_quality_floor,
					&obj_quality_mode, &obj_quality_scale, &obj_quality_floor,
					&obj_quality_center_weight, &obj_quality_geom_weight, &obj_quality_phys_weight,
					...
					&flux_refine_mode, &flux_refine_delta_param_index, &flux_refine_gate_param_index,
					&flux_refine_detach_base, &flux_refine_loss_scale, &flux_refine_gate_loss_scale,
					&flux_refine_final_loss_scale, &flux_refine_delta_norm_scale, &flux_refine_gate_margin_norm))
...
static PyMethodDef CIANNAMethods[] = {
	...
	{ "channel_gate", (PyCFunction)py_channel_gate, METH_VARARGS | METH_KEYWORDS, "Add a trainable per-channel sigmoid gate attention layer"},
