...
global data_path
global FULL_IMAGE_PATH, PRIMARY_BEAM_PATH, TRAINING_CATALOG_PATH, TRUTH_CATALOG_PATH, REF_MODEL_PATH

script_dir = Path(__file__).resolve().parent
raw_data_dir = Path(os.environ.get("SDC1_RAW_DATA_DIR", "/shared/main/dengyuhe/SDC1_YOLO_OBB/raw_data/560Mhz-1kh")).expanduser()
aux_data_dir = script_dir / "SDC1_data"

data_path = str(raw_data_dir) + os.sep
FULL_IMAGE_PATH = str(raw_data_dir / "sdc1_560MHz_1000h.fits")
PRIMARY_BEAM_PATH = str(raw_data_dir / "PrimaryBeam_560MHz.fits")
TRAINING_CATALOG_PATH = str(raw_data_dir / "TrainingSet_560MHz.txt")
TRUTH_CATALOG_PATH = str(raw_data_dir / "True_560MHz.txt")
REF_MODEL_PATH = str(aux_data_dir / "YOLO_CIANNA_ref_SDC1_s480k_MINERVA_Cornu2024.dat")


def require_data_file(path):
	if(not os.path.isfile(path)):
		raise FileNotFoundError(
			"Required SDC1 data file not found: %s\n"
			"Expected raw data directory: %s\n"
			"Set SDC1_RAW_DATA_DIR to override this location."%(path, data_path)
		)
	return path
...
hdul     = fits.open(require_data_file(FULL_IMAGE_PATH))
full_img = hdul[0].data[0,0]
wcs_img  = WCS(hdul[0].header)

#Load primary beam for flux correction
hdul_beam = fits.open(require_data_file(PRIMARY_BEAM_PATH))
data_beam = hdul_beam[0].data[0,0]
wcs_beam  = WCS(hdul_beam[0].header)


#Load the source catalogs
require_data_file(TRAINING_CATALOG_PATH)
require_data_file(TRUTH_CATALOG_PATH)
